...
| 명단 재업로드 | 기존 명단과 `이름 + 성별 + 나이`가 모두 같은 인원은 최신 입력값으로 덮어쓰고, 중복되지 않는 인원은 추가한다. |
| 목표 그룹 수 | 자동 계산하지 않는다. 사용자가 자동 그룹화 전에 목표 그룹 수를 먼저 입력한다. 시스템은 총 인원을 목표 그룹 수로 균등 분배하고, 나머지 인원을 앞 그룹부터 하나씩 배정한 기본 정원을 제시한다. 예: 5명, 3그룹은 `2, 2, 1`이다. 생성된 각 그룹의 목표 인원 수는 사용자가 개별적으로 조정할 수 있으며, 합계는 실제 인원 수와 같아야 한다. |
