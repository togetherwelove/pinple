...
export function GroupUnassignDialog({
  groupName,
  memberCount,
  onCancel,
  onConfirm,
}: GroupUnassignDialogProps) {
  return (
    <div
      aria-modal="true"
    ...
    >
      <section className="w-full max-w-sm border border-[var(--border)] bg-[var(--surface)] p-5 shadow-lg">
        <h2 className="font-semibold">
          {UI_LABELS.moveGroupToUnassignedTitle}
        </h2>
        <p className="mt-2 text-sm text-[var(--muted)]">
          {UI_MESSAGES.groupUnassignConfirmation(groupName, memberCount)}
        </p>
        <div className="mt-5 flex justify-end gap-2">
          ...
          <button
            ...
            type="button"
          >
            {UI_LABELS.moveGroupToUnassigned}
          </button>
        </div>
      </section>
    </div>
